...
- The demo data set's two external endpoints move to `203.0.113.0/24` (RFC 5737, reserved for documentation). They were `203.0.113.9` and `203.0.113.34` — invented, but both land in real allocated space (Google and Linode), and the demo presents them as an unknown camera's cloud endpoint and an integrator's RDP ingress. Nothing should be pointing a security example at somebody's actual address.
