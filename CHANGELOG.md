...
## v0.19.1 — 2026-09-13
- **`--allowed-host`, because the tool was refusing the access method it recommends.** `bind.py` tells you to reach a remote collector with `tailscale serve`, and then the DNS-rebinding check added in v0.16.0 refused every request that arrived that way with a `421`. Tailscale Serve preserves the hostname the browser typed, so a loopback bind sees a tailnet name it has no way to guess. The flag names it. Repeatable, and every other name is still refused — this widens the allowlist by exactly one entry, it does not switch the check off. An `ssh -L` forward never needed it: that arrives as `127.0.0.1`.
- The name is accepted with or without a port. The command line invites you to paste the URL you type, which carries `:8443`, and the incoming Host has its port stripped before comparison — so storing it verbatim would have produced an allowlist entry that silently never matched. One `bare_host()` now normalises both sides, so they cannot drift.
- **The 421 says what it refused and what to do about it.** It read "Unrecognised Host header — refused." with no indication of which name, which sends you looking for a network fault that isn't there. It now quotes the name and hands you the exact `--allowed-host` value. The value came from the caller, so echoing it tells an attacker nothing they did not already send. The 403 names the Origin the same way.
- Naming a tunnel host prints a startup notice. It is the moment the interface stops being reachable only by the person at the machine, and everything else in `bind.py` says so out loud; this did not. Not a refusal — a tunnel that authenticates is the right answer — but a statement of fact, and a reminder that the tunnel is what is doing the authenticating.
- `INSTALL.md`'s remote-access section now shows both commands. It showed `tailscale serve` alone, which does not work on its own.
- Twelve tests in `tests/test_hostile_input.py`, the ones that matter being that a *different* tailnet name, a suffix attack (`laptop.tailnet-name.ts.net.evil.example`) and a foreign Origin are all still refused with the flag in use. The suite is now 188.
